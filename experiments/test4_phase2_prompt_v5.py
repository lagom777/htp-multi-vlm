"""Phase 2 — 새 prompt v5 (SizeHint + Region priming) × 4장 단일 Qwen."""
import sys, time
import os
...
def make_r1_v5_en(cat):
    """v5: SizeHint + Region priming (영역별 살피기)."""
...
Identify all visible objects, scanning the image systematically:
- Top-left, top-center, top-right regions
- Middle-left, middle-center, middle-right regions
- Bottom-left, bottom-center, bottom-right regions

Include:
- Objects of various sizes (large + small details)
- Overlapping objects as separate
- Only what you actually see (no guessing)
...
print("=== Qwen 단일 v5 (Region priming) 4장 ===")
tot = Counter()
for cat, img in TEST:
    img_path = fp(BASE_IMG, [TS_MAP[cat], img])
    gt = load_gt(cat, img); gtn = len(gt)
    try:
        raw = call_vlm("qwen", make_r1_v5_en(cat), img_path=img_path, temp=0.2)
    except: raw = ""
...
print(f"=== v5 완료 Acc {acc*100:.1f}% F1 {f1*100:.1f}% Recall {rec*100:.1f}% Prec {prec*100:.1f}% 9-Pos {pacc*100:.1f}% 환각 {tot['fp']} ===")
